...
### **Command Flow**
1. **Routing** → **Selection** → **Orchestration** → **Execution** → **Verification** → **Documentation**
...
- **CLAUDE.md Imports**: Use [CLAUDE.md Import System](../knowledge/reference/claude-md-imports.md) for modular command loading
- **Selective Loading**: Import specific command categories based on task requirements
- **Memory Verification**: Use `/memory` command to verify loaded command context
- **Context Optimization**: Strategic imports reduce cognitive load while maintaining functionality
...
@.claude/commands/cores/README.md          # Core processors

# Targeted Workflows
@.claude/commands/orchestration/           # Workflow coordination
@.claude/commands/intelligence/            # Smart operations
```
...
- `operacionales/` → Distributed across functional categories
- `orquestadores/` → Mainly to `orchestration/`
- `comportamiento/` → Mainly to `behavior/`
- `meta/` → Preserved as `meta/`
- `decision/` → Distributed across `core-routing/`, `orchestration/`, `selection/`, `mapping/`
