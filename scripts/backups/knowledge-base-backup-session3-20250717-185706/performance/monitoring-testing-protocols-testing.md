  ...
  performance-testing:
    ...
    steps:
      ...
      - name: Check bundle size regression
        run: ./scripts/performance/check-bundle-regression.sh
      ...
      - name: Comment PR with performance results
        ...
        with:
          script: |
            const fs = require('fs');
            const report = JSON.parse(fs.readFileSync('.performance-baseline/latest-report.json', 'utf8'));
            
            const body = `## 📊 Performance Analysis
            
            ### Bundle Sizes
            - **JavaScript**: ${report.bundles.javascript.sizeKB}KB (${report.bundles.javascript.change}% change)
            - **CSS**: ${report.bundles.css.sizeKB}KB (${report.bundles.css.change}% change)
            
            ### Budget Compliance
            - **JavaScript**: ${report.bundles.javascript.withinBudget ? '✅' : '❌'} (Budget: 500KB)
            - **CSS**: ${report.bundles.css.withinBudget ? '✅' : '❌'} (Budget: 100KB)
            
            ### Status
            ${report.violations === 0 && report.regressions === 0 ? '✅ All performance checks passed' : '❌ Performance issues detected'}
            `;
